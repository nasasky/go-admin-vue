...
export const getUserList = (params: IUser.Query) => {
  return http.get<IPage<IUser.Info>>(ADMIN_MODULE + `/employee/list`, params, { loading: false });
};
...
export const addUser = (params: IUser.Form) => {
  return http.post(ADMIN_MODULE + `/employee/add`, params);
};
...
export const editUser = (params: IUser.Form) => {
  return http.put(ADMIN_MODULE + `/employee/update`, params);
};
...
export const deleteUser = (params: { ids: number[] }) => {
  return http.delete(ADMIN_MODULE + `/employee/delete`, params);
};
...
export const getUserDetailApi = (params: { id: string }) => {
  const { id } = params;
  return http.get<IUser.Info>(ADMIN_MODULE + `/employee/detail/${id}`);
};
