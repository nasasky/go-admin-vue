...
export const getAllBreadcrumbList = (
  ...
  for (const item of menuList) {
    result[item.rule] = [...parent, item];
    if (item.children) {
      getAllBreadcrumbList(item.children, result[item.rule], result);
    }
  }
  return result;
};
...
/**
 * @description 递归查询当前 rule 所对应的菜单对象 (该函数暂未使用)
 * @param {Array} menuList 菜单列表
 * @param {String} rule 当前访问地址
 * @returns {Object | null}
 */
export function findMenuByPath(menuList: Menu.MenuOptions[], rule: string): Menu.MenuOptions | null {
  for (const item of menuList) {
    if (item.rule === rule) return item;
    if (item.children) {
      const res = findMenuByPath(item.children, rule);
      if (res) return res;
    }
  }
  return null;
