  ...
  <div class="table-box">
    <ProTable ref="proTableRef" title="教师统计" :indent="20" :columns="columns" :search-columns="searchColumns"
      ...
      <template #tableHeader="scope">
        <el-button type="primary" v-auth="'teacher.statistics.create'" :icon="CirclePlus"
          @click="openAddEdit('新增教师统计')">
          新增
        </el-button>
        <el-button v-auth="'teacher.statistics.remove'" type="danger" :icon="Delete" plain :disabled="!scope.isSelected"
          @click="batchDelete(scope.selectedListIds)">
          批量删除
        </el-button>
        <el-button v-auth="'teacher.statistics.import'" type="primary" :icon="Upload" plain @click="importData"> 导入
        </el-button>
      ...
      <template #operation="{ row }">
        <el-button v-auth="'teacher.statistics.update'" type="primary" link :icon="EditPen"
...
import { getrenueListApi } from '@/api/modules/order/orderList';
...
const columns: ColumnProps<ITeacherStatistics.Row>[] = [
  ...
  { prop: 'no', label: '订单号' ,width:300},
  { prop: 'user_name', label: '下单用户' },
  {
    prop: 'status',
    label: '订单状态',
    tag: true,
    enum: optionsStore.getDictOptions('order_status'),
    fieldNames: {
      label: 'codeName',
      value: 'code',
...
const getTableList = (params: ITeacherStatistics.Query) => {
  ...
  return getrenueListApi(newParams);
};
