<template>
  <div class="dialog-table-box">
    <el-dialog
      v-model="visible"
      :title="`字典: ${info?.type_name}(${info?.type_code})`"
      width="90%"
      ...
      >
        ...
        <template #tableHeader="scope">
          <el-button
            type="primary"
            :icon="CirclePlus"
            @click="openAddEdit('新增字典', { id: info?.id, sort: 1, callback_show_style: 'primary' }, true)"
          >
            新增字典
          </el-button>
        ...
        <template #is_show="{ row }">
          <el-tag :type="row.is_show === 'T' ? 'success' : 'danger'">
            {{ yesNoOptionsLabel(row?.is_show) }}
          </el-tag>
        </template>

        <template #callback_show_style="{ row }">
          <el-tag :type="row.callback_show_style">
            {{ tagsTypeOptionsLabel(row?.callback_show_style) }}
          </el-tag>
        </template>
        ...
        <template #operation="{ row }">
          <el-button
            type="primary"
            link
            :disabled="row.is_lock === 'T'"
            :icon="EditPen"
            @click="openAddEdit('编辑字典', row, false)"
          >
            编辑
          </el-button>
          <el-button type="primary" link :disabled="row.is_lock === 'T'" :icon="Delete" @click="deleteInfo(row)"> 删除 </el-button>
        </template>
      </ProTable>
...
const show = (params: IDict.DictType) => {
  initParam.id = params.id as number;
  info.value = params;
  visible.value = true;
};
...
const columns: ColumnProps<IDict.Dict>[] = [
  ...
  { prop: 'alias', label: '别名' },
  { prop: 'code_name', label: '名称' },
  { prop: 'code', label: '字典值' },
  { prop: 'is_show', label: '显示', enum: yesNoOptions, width: 80 },
  { prop: 'callback_show_style', label: '回显样式', width: 100 },
